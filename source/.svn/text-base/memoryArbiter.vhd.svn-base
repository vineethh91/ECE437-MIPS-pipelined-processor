...
--    pipelineIFIDStall <= '0' when ((state = LOAD_WORD_BUSY) or (state = STORE_WORD_BUSY)) else '1';
--    pipelineIDEXStall <= '0' when ((state = LOAD_WORD_BUSY) or (state = STORE_WORD_BUSY)) else '1';
--    pipelineEXMEMStall <= '0' when ((state = LOAD_WORD_BUSY) or (state = STORE_WORD_BUSY)) else '1';
--    pipelineMEMWBStall <= '0' when ((state = LOAD_WORD_BUSY) or (state = STORE_WORD_BUSY)) else '1';

    pipelineIFIDStall <= '1' when (((state = FETCH_INSTRUCTION_BUSY) and (memstate = "10")) or ((state = LOAD_WORD_BUSY) and (memstate = "10")) or ((state = STORE_WORD_BUSY) and (memstate = "10"))) else '0';
    pipelineIDEXStall <= '1' when (((state = FETCH_INSTRUCTION_BUSY) and (memstate = "10")) or ((state = LOAD_WORD_BUSY) and (memstate = "10")) or ((state = STORE_WORD_BUSY) and (memstate = "10"))) else '0';
    pipelineEXMEMStall <= '1' when (((state = FETCH_INSTRUCTION_BUSY) and (memstate = "10")) or ((state = LOAD_WORD_BUSY) and (memstate = "10")) or ((state = STORE_WORD_BUSY) and (memstate = "10"))) else '0';
    pipelineMEMWBStall <= '1' when (((state = FETCH_INSTRUCTION_BUSY) and (memstate = "10")) or ((state = LOAD_WORD_BUSY) and (memstate = "10")) or ((state = STORE_WORD_BUSY) and (memstate = "10"))) else '0';


    aiMemWait <= '0' when (((state = FETCH_INSTRUCTION_BUSY) and memstate = "10") ) else '1';--or ((state = LOAD_WORD_BUSY) and memstate = "10") or ((state = STORE_WORD_ACCESS) and (adMemWrite /= '1'))
